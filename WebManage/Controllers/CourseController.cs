    ...
    public class CourseController : Controller
    {
        ...
        public IActionResult QueryWorkSource(string startStr, string endStr)
        {
            ...
            var list = _currencyService.DbAccess().Queryable<C_Course_Work, C_Subject, C_Project, C_Class, C_Contrac_User, C_Room,sys_user>((cou, sub, pro, cla, conU, room,ta) => new object[] { JoinType.Left,cou.SubjectId==sub.SubjectId,JoinType.Left,cou.ProjectId==pro.ProjectId, JoinType.Left, cou.ClasssId==cla.ClassId, JoinType.Left,
                ...
                .Where(cou =>(cou.StudyMode==1|| cou.StudyMode == 2 || cou.StudyMode == 4|| cou.StudyMode == 8) &&cou.AT_Date.ToString("yyyy-MM-dd") == DateTime.Now.ToString("yyyy-MM-dd"))
        ...
        public IActionResult Todaymock() {
            return View();
        }
        ...
        /// </summary>
        /// <returns></returns>
        public IActionResult QueryTodayMock() {
            ResResult rsg = new ResResult() { code = 200, msg = "获取成功" };
            var list = _currencyService.DbAccess().Queryable<C_Course_Work, C_Subject, C_Project, C_Class, C_Contrac_User, C_Room, sys_user>((cou, sub, pro, cla, conU, room, ta) => new object[] { JoinType.Left,cou.SubjectId==sub.SubjectId,JoinType.Left,cou.ProjectId==pro.ProjectId, JoinType.Left, cou.ClasssId==cla.ClassId, JoinType.Left,
                cou.StudentUid ==conU.StudentUid,JoinType.Left,cou.RoomId==room.Id,JoinType.Left,cou.TeacherUid==ta.User_ID})
    .Where(cou => cou.StudyMode ==5&& cou.AT_Date.ToString("yyyy-MM-dd") == DateTime.Now.ToString("yyyy-MM-dd"))
    .OrderBy(cou => DateTime.Parse(cou.AT_Date.ToString("yyyy-MM-dd") + " " + cou.StartTime))
    .Select<CourseWorkModel>((cou, sub, pro, cla, conU, room, ta) => new CourseWorkModel { Id = cou.Id, StudentUid = cou.StudentUid,Student_Name=conU.Student_Name,Work_Title = cou.Work_Title, AT_Date = cou.AT_Date, StartTime = cou.StartTime, EndTime = cou.EndTime, StudyMode = cou.StudyMode, TeacherName = ta.User_Name, RoomName = room.RoomName }).ToList();
            rsg.code = 0;
            rsg.data = list;
            return Json(rsg);
        }

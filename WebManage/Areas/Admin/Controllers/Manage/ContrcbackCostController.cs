...
{
    [Authorize]
    public class ContrcbackCostController : BaseController
    {
        private RedisConfig redisConfig;
        private ICurrencyService _currencyService;
        private IC_ContracService _contrac;
        public ContrcbackCostController(ICurrencyService currencyService, IC_ContracService contrac)
        {
        ...
        [UsersRoleAuthFilter("T-353", FunctionEnum.Have)]
        ...
        public IActionResult GetDataSource(int contraProperty, int studymode, DateTime? startime, DateTime? endtime, int page = 1, int limit = 10)
        {
            ...
            var list = _currencyService.DbAccess().Queryable(@"(select c.*,contracU.Student_Name,contracU.Student_Phone,cc.User_Name as CCUserName,cls.BackCost,cls.IsOrg from C_Contrac_Child c inner join C_Contrac_Child_ConstBack cls on c.Contra_ChildNo=cls.Contra_ChildNo left join C_Contrac_User contracU on c.StudentUid=contracU.StudentUid
                ...
                .Where("(orginSql.Contrac_Child_Status=" + (int)ConstraChild_Status.BackPay + " or orginSql.Contrac_Child_Status=" + (int)ConstraChild_Status.BackPayOk + " or orginSql.Contrac_Child_Status=" + (int)ConstraChild_Status.BackPayReject + ")")
        ...
        /// <returns></returns>
        public IActionResult ChildContrcbackCostAudit(string Contra_ChildNo,bool through) {
            ResResult reg = new ResResult();
            var userId = this.User.Claims.FirstOrDefault(c => c.Type == "ID")?.Value;
            if (!string.IsNullOrEmpty(Contra_ChildNo))
            {
                reg = _contrac.ContrcBackCostPartAudit(Contra_ChildNo, through, userId);
            }
            else {
                reg.msg = "缺少参数";
                reg.code = 300;

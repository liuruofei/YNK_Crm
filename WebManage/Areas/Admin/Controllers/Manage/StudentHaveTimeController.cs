...
{
    [Authorize]
    public class StudentHaveTimeController : BaseController
    {
        private ICurrencyService _currencyService;
        public StudentHaveTimeController(ICurrencyService currencyService)
        {
        ...
        [UsersRoleAuthFilter("V-360", FunctionEnum.Have)]
        ...
        public IActionResult QueryNameAll(string title)
        {
            ...
            {
                listSerchName = _currencyService.DbAccess().Queryable<C_Contrac_User>().Where(c=>c.Student_Name.Contains(title)).Select<SeachUNameModel>(c=>new SeachUNameModel { 
                 Name=c.Student_Name
                }).ToList();
        ...
        [UsersRoleAuthFilter("V-360", FunctionEnum.Have)]
        public IActionResult QueryStudentTime(string teacherNames, DateTime? endtime = null, DateTime? starttime = null)
        {
            string[] names = teacherNames.Split(",");
            List<TeacherTimeWorkModel> tdlistTime = new List<TeacherTimeWorkModel>();
            var students = _currencyService.DbAccess().Queryable<C_Contrac_User>().Where(c => names.Contains(c.Student_Name)).ToList();
            if (names != null && names.Length > 0)
            {
                var studetUids = students.Select(n => n.StudentUid).ToList();
                ...
                var classChild = _currencyService.DbAccess().Queryable<C_Contrac_Child>().Where(con => studetUids.Contains(con.StudentUid) && con.StudyMode == 2 && con.Contrac_Child_Status != contracStatus).GroupBy(con=>new {con.ClassId,con.StudentUid})
                    .Select<C_Contrac_Child>(con=>new C_Contrac_Child {ClassId=con.ClassId,StudentUid=con.StudentUid}).ToList();
                List<int> contansClassIds = classChild.Select(con=>con.ClassId).ToList();
                //查询学生已排课课程
                string sql = "";
                if (contansClassIds != null && contansClassIds.Count > 0)
                {
                    sql += " or cour.ClasssId in(" + string.Join(",", contansClassIds) + ")";
                }
                List<CourseWorkModel> listCourseWork = _currencyService.DbAccess().Queryable<C_Course_Work>("cour").AddJoinInfo("C_Contrac_User","u", "cour.StudentUid=u.StudentUid",JoinType.Left)
                    .Where("(cour.StudentUid in(" + string.Join(",", studetUids) + ") "+sql+ " ) and CAST(cour.AT_Date as date) >=CAST(@starttime as date) and CAST(cour.AT_Date as date)<=CAST(@endtime as date)")
                    .AddParameters(new { starttime = starttime.Value, endtime = endtime.Value})
                    .Select<CourseWorkModel>("cour.Id,cour.AT_Date,cour.CourseTime,cour.EndTime,cour.StartTime,cour.StudyMode,u.Student_Name,cour.StudentUid,cour.ClasssId,cour.Work_Title").ToList();
                tdlistTime.ForEach(it =>
                {
                    ...
                    if (listCourseWork != null && listCourseWork.Count > 0)
                    {
                        ...
                        students.ForEach(tc =>
                        {
                            var currtClassIds = classChild.Where(con => con.StudentUid == tc.StudentUid).Select(con => con.ClassId).ToList();
                            List<StudentWorkPlanModel> cutTeacherPlan = new List<StudentWorkPlanModel>();
                            var filterCosurWork = listCourseWork.FindAll(iv => iv.AT_Date.ToString("yyyy-MM-dd") == it.WorkDate.ToString("yyyy-MM-dd") && (iv.StudentUid == tc.StudentUid || currtClassIds.Contains(iv.ClasssId))).ToList();
                            ...
                            it.ListTeacherTime.Add(tc.Student_Name, cutTeacherPlan);
                        });
                        ...
                        students.ForEach(tc =>
                        {
                            it.ListTeacherTime.Add(tc.Student_Name,new List<StudentWorkPlanModel>(){});
                        });
                     }
                });
            }

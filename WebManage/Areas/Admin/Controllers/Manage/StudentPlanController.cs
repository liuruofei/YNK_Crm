    ...
    public class StudentPlanController : BaseController
    {
        ...
        public IActionResult QueryStudentPlan(int studentUid, int isAdd, DateTime? endtime = null, DateTime? starttime = null)
        {
            ...
            List <CourseWorkModel> listCourseWork = _currencyService.DbAccess().Queryable<C_Course_Work,sys_user, C_Room>((cour,ta,room)=>new object[] { JoinType.Left,cour.TeacherUid==ta.User_ID, JoinType.Left, cour.RoomId ==room.Id })
                ...
                .Select<CourseWorkModel>((cour,ta, room) =>new CourseWorkModel { Id=cour.Id,AT_Date=cour.AT_Date,ClasssId=cour.ClasssId,Comment=cour.Comment,Contra_ChildNo=cour.Contra_ChildNo,CourseTime=cour.CourseTime,CreateTime=cour.CreateTime,
             CreateUid=cour.CreateUid,EndTime=cour.EndTime,ProjectId=cour.ProjectId,RangTimeId=cour.RangTimeId,RoomId=cour.RoomId,StartTime=cour.StartTime,StudentUid=cour.StudentUid,StudyMode=cour.StudyMode,
             SubjectId=cour.SubjectId,TA_Uid=cour.TA_Uid,TeacherName=ta.User_Name,TeacherUid=cour.TeacherUid,Work_Stutas=cour.Work_Stutas,Work_Title=cour.Work_Title,Status=cour.Status,
             UpdateTime=cour.UpdateTime,UpdateUid=cour.UpdateUid,RoomName=room.RoomName
            }).ToList();
            //查询学生任务计划
                ...
                if (listCourseWork != null && listCourseWork.Count > 0)
                {
                    ...
                    if (filterCosurWork != null && filterCosurWork.Count > 0) {
                        filterCosurWork.ForEach(iv => {
                            ...
                            if(!string.IsNullOrEmpty(iv.Comment))
                ...
                if (listTask != null && listTask.Count > 0) {
                    ...
                    if (filterlistTask != null && filterlistTask.Count > 0) {
                        filterlistTask.ForEach(iv => {
                            ...
                            {
                                it.ChouciComent += iv.Task_Name + ",平均反应时间" + iv.AvgSpell + ",正确率" + iv.Accuracy + ",正确数量" + iv.isRightCount;
                            }
                            else {
                                it.ChouciComent += iv.Task_Name + ",拼写单词-单词数量" + iv.Words_Count + ",正确率" + iv.Accuracy + ",正确数量" + iv.isRightCount;
                            }
                        });
                    }
                }
        ...
        public IActionResult Find(int ID)
        {
            ...
            if (ID > 0)
            {
                list = _currencyService.DbAccess().Queryable<C_Student_Work_Plan>().Where(f => f.Id == ID).First();
            }
            else
            {
                list = new { };
